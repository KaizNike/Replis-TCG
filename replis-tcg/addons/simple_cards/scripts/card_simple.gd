...
var current_texture: CompressedTexture2D = null: ##This is the texture you see
	set(v):
		current_texture = v
		set_tooltip()
var front_texture: CompressedTexture2D = null ##This is the texture of the front of the card
...
@export var start_face: face = face.BACK  ##Sets on what face the card spawns
...
static func spawn_card(_card_resource: CardResource = TEST_RESOURCE, _name: String = "") -> CardSimple:
	var _card: CardSimple = CARD_SCENE.instantiate()
...
func set_required():
	...
	front_texture = card_resource.card_face
	back_texture = card_resource.card_back
		
	match start_face:
		face.BACK: current_texture = back_texture
		face.FRONT: current_texture = front_texture
	set_tooltip()
...
func set_tooltip():
	...
	match current_texture:
		back_texture:
			tooltip_text = "Press " + CardGlobal.use_key + " to flip"
		front_texture:
			tooltip_text = card_resource.card_description


func on_button_down() -> void:
	if Input.is_action_pressed(CardGlobal.move_key):
		initial_position = global_position
		
		dragging = true
		dragging_offset = get_global_mouse_position() - position
	elif Input.is_action_pressed(CardGlobal.use_key):
...
func flip_to_other_face():
	if current_texture == back_texture:
		card_texture.flip_card(front_texture, flip_duration)
		current_texture = front_texture
	elif current_texture == front_texture:
